...
ActiveSupport::Notifications.subscribe 'sessions.destroy' do |name, start, finish, id, payload|
  Event.create(:eventable_id=>payload[:user_id],:eventable_type=>"Sign_out", :user_id=>payload[:user_id])
end

ActiveSupport::Notifications.subscribe 'sessions.create' do |name, start, finish, id, payload|
  Event.create(:eventable_id=>payload[:user_id],:eventable_type=>"Sign_in", :user_id=>payload[:user_id])
end
